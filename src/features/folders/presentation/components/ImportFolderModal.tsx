...
export function ImportFolderModal({ visible, onClose, folderId, onSuccess }: ImportFolderModalProps) {
    ...
    const handlePickFile = async () => {
        ...
        if (!result.canceled && result.assets.length > 0) {
            ...
            setFile({ name: asset.name, uri: asset.uri, mimeType: asset.mimeType ?? undefined, size: asset.size ?? undefined });
        }
    };
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            const errors: string[] = data?.data?.errors ?? data?.errors ?? [];
    ...
    const fileIcon = file?.name.endsWith('.json') ? 'doc.text.fill' : file?.name.endsWith('.xml') ? 'doc.badge.gearshape.fill' : 'doc.fill';
                    ...
                    <TouchableWithoutFeedback>
                        <View style={styles.container}>
                            ...
                            >
                                {file ? (
                                    <>
                                        ...
                                        <TouchableOpacity onPress={() => setFile(null)} disabled={loading}>
                                            <ThemedText style={styles.removeText}>{i18n.t('common.actions.remove')}</ThemedText>
                                        </TouchableOpacity>
                                    </>
                                ) : (
                                    <>
                                        ...
                                        <ThemedText style={styles.dropZoneHint}>.json, .xml</ThemedText>
                                    </>
                                )}
                            </TouchableOpacity>
                            ...
                            <View style={styles.actions}>
                                ...
                                />
                                <Button
                                    title={i18n.t('folders.import_modal.action')}
